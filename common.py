...
def test_for_dmnet(args, test_name, test_loader, net, excel_file, save_dir=None):
    psnr_list = []
    ssim_list = []
        ...
        for sig in [0, 1.5]: # default=[0, 1.5, 3]
            ...
            with torch.no_grad():
                ...
                for index, test_name in enumerate(test_Names):
                    ...
                    save_dir = result_dir.joinpath(test_name)
                    save_dir.mkdir(exist_ok=True)
                    psnr, ssim = test_for_dmnet(args, test_name, test_loader, net, excel_file, save_dir)
