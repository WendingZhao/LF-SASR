...
"EPI use vanilla conv, use IKC's estimator for degradation estimation"
...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.deep_conv = CascadeGroups(n_group, n_block, self.angRes, channels)
        self.up_sample = nn.Sequential(
            nn.Conv2d(channels, channels * self.factor ** 2, kernel_size=1, stride=1, padding=0, bias=False),
    ...
    def forward(self, lf, info=None):
        ############
        blur = info[1] / 4.0 if self.task == 'BlindSSR' else (info[1] + 2.0) / 4.0
        noise = info[2] / 75.0 if self.task == 'BlindSSR' else (info[2] + 30.0) / 75.0
        ############
        blur = (blur+1e-4).view(-1, 1, 1, 1).repeat(1, 1, self.angRes, self.angRes).to(lf.device)
        noise = (noise+1e-4).view(-1, 1, 1, 1).repeat(1, 1, self.angRes, self.angRes).to(lf.device)
        ...
        sr = self.up_sample(buffer)
        sr = rearrange(sr, '(b u v) c h w -> b c u v h w', u=u, v=v)
...
class PixelShuffle1D(nn.Module):
    """
    1D pixel shuffler
    Upscales the second last dimension (i.e., H) of a tentor by reducing its channel length
    inout: x of size [b, factor*c, h, w]
    output: y of size [b, c, h*factor, w]
    """
...
class get_loss(nn.Module):
    ...
    def forward(self, out, HR, info=None):
        SR = out['SR']
